...
function deployments(): Record<string, string> {
  ...
  if (!existsSync(p)) throw new Error(`No deployments for ${NETWORK}. Run contract/deploy.ts first.`)
...
async function main() {
  ...
  if (royaltyBps > 2500) throw new Error(`ALEA_ROYALTY_BPS=${royaltyBps} exceeds the contract's 25% cap.`)
  ...
  const est = await tezos.estimate.transfer(factory.methodsObject.deploy(params).toTransferParams())
  ...
  console.log(`  mint storage burn  ${(buyEst.burnFeeMutez / 1_000_000).toFixed(6)} tez  (${buyEst.storageLimit} bytes)`)
