...
function loadCode(name: Name): unknown {
    ...
    try {
        ...
        throw new Error(`No compiled code at ${p}\n  Run: python3 contract/build.py`);
    }
}
...
function roles(): { admin: string; treasury: string; agent: string } {
    ...
    for (const [name, value] of [
        ...
        if (!value) {
            ...
            console.error(
                "  Set every role explicitly. A collapsed key means one leak takes all of them.",
            );
            process.exit(1);
        }
    }
...
async function main() {
    ...
    const storageFor = (name: Name): Record_ => {
        switch (name) {
            ...
            case "factory": {
                ...
                if (!resolverAddress) {
                    throw new Error(
                        "The factory needs a resolver. Deploy that first, or pass --only resolver.",
                    );
                }
                return {
                    administrator: admin,
    ...
    console.log(`  gas caps  ${limits.gasPerOperation} per op, ${limits.gasPerBlock} per block`);
    ...
    for (const name of targets) {
        ...
        try {
            ...
            if (estimate.opSize && estimate.opSize > MAX_OPERATION_BYTES) {
                console.error(
                    `  ✗ operation is ${estimate.opSize} bytes, over the ${MAX_OPERATION_BYTES} cap`,
                );
                process.exit(1);
            }
        } catch (e) {
