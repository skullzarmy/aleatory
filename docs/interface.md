...
cannot deliver is skipped and goes to the seller instead, so one bad address
never makes a collection unsellable — but `royalties` has no setter, so a
skipped recipient forfeits that share on every sale, forever. A contract
that accepts the transfer and then fails while handling it still reverts
the sale.
