...
- ~~**Who mints, and who may write a token's image URI.**~~ Resolved 2026-08-23, and the two halves separated. **Minting** is the collector, in their own `buy` operation. **Publishing the metadata** is an authorised render provider through `set_token_metadata`, which replaces one token's metadata pointer, once. A multisig admin was considered and set aside — admin is just an address, so pointing it at one later is a transfer, not a migration. See [decisions.md](decisions.md).
