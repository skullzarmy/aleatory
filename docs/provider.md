...
A provider draws the image for every piece minted from a generator that names
...
Leaving is `deregister`, callable only by your own operator. Generators
...
their generator half-rendered will switch, and switching is one call on their
...
charge and pays that, so a cut reaches every generator immediately and nobody
...
```
generator.render.provider  →  provider contract
...
work: one `set_agent` on your own contract and every generator using you
follows immediately, with no artist doing anything. If generators trusted
...
| `ALEA_ROUTER_ADDRESS` | the platform's index, how generators are found | the daemon |
...
generator's provider address off the chain, reads `endpoint` out of that
...
| leaves the generator scan alone | that scan runs at most once a minute, and a tap cannot change it |
...
A piece needs rendering when its `token_info[""]` still equals the generator's
...
Generators come from two places, because neither alone is complete. A
factory-deployed generator names its provider in its *initial storage* and
never emits `set_provider`, so an event scan alone never sees a new generator
at all. A generator that switched to you later does emit one. Both are
checked against the generator's own storage afterwards, which is the only
...
A generator declares what its generator needs under `aleatory:libraries`, and
...
generator using you follows immediately, because they ask `get_agent()` live
